...
BreezeBlock {
    ...
    main: ColumnLayout {
        ...
        BreezeLabel {
            ...
            text: i18ndp("plasma_lookandfeel_rosa.fresh.lookandfeel", "in 1 second", "in %1 seconds", root.remainingTime);
            visible: true
        }
        ...
        states: [
            State {
                ...
                PropertyChanges { target: actionLabel; text: i18ndc("plasma_lookandfeel_rosa.fresh.lookandfeel","Dialog heading, confirm shut down, not a status label","Shutting down") }
                PropertyChanges { target: actionIcon; source: "system-shutdown" }
            },
            State {
                name: "logout"
                PropertyChanges { target: root; currentAction: logoutRequested }
                PropertyChanges { target: actionLabel; text: i18ndc("plasma_lookandfeel_rosa.fresh.lookandfeel","Dialog heading, confirm log out, not a status label","Logging out") }
                PropertyChanges { target: actionIcon; source: "system-log-out" }
            },
            State {
                name: "reboot"
                PropertyChanges { target: root; currentAction: rebootRequested }
                PropertyChanges { target: actionLabel; text: i18ndc("plasma_lookandfeel_rosa.fresh.lookandfeel","Dialog heading, confirm reboot, not a status label","Rebooting") }
    ...
    controls: Item {
        ...
        states: [
            State {
                name: "shutdown"
                PropertyChanges { target: commitButton; text: i18nd("plasma_lookandfeel_rosa.fresh.lookandfeel","Shut down") }
            },
            State {
                name: "logout"
                PropertyChanges { target: commitButton; text: i18nd("plasma_lookandfeel_rosa.fresh.lookandfeel","Log out") }
            },
            State {
                name: "reboot"
                PropertyChanges { target: commitButton; text: i18nd("plasma_lookandfeel_rosa.fresh.lookandfeel","Reboot") }
            }
        ]
    }
}
