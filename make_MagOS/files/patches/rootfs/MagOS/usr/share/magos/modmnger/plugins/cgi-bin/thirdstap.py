...
#step3_help = _('Last step, bootloader installation. ').encode('UTF-8')
step3_help = 'Последний шаг, установка загрузчика. Выберте загрузчик и диск на который нужно его установить. Если нужного диска нет в списке возможно он не смонтирован, либо файлы загрузчика не скопированы на нужный диск, если так вы можете вернуться к шагу 2. '
...
def bootloader_install():
	...
	command = ('beesu  xterm -e  ' + boot_mount  + loader  )
	ret = subprocess.call(command, shell=True)
	if ret != 0:
		dialog_text = ('<br>Sorry, bootloader install -  FAIL!!!? code:' + ret)
	return  dialog_text
...
print '''
<div class="set"><table id="main_head" width="320"> <tr><td>
<form action="/cgi-bin/thirdstap.py">'''
print ' <button type="submit">' + update_ + '</button></form>'
print '</td><td>'
print '<form action="/cgi-bin/secondstap.py"><button type="submit">' + prev_ + '</button></form>'
print '</td><td>'
print '<form action="/cgi-bin/install_help.py"><button type="submit">' + help_ + '</button></form>'
print '</td></tr></table></div>'

print '<form action="/cgi-bin/thirdstap.py" method="post" name=loader_form>'
print '<h3>' + loader_header + '</h3>'
	
print '<table  id="loader-table" border="1" width="100%">'
print '<tr><td align="center">' + flag + '</td><td align="center">' +  loader_ +  '</td><td align="center">' + script + '</td>' 
print '<td align="center"> MBR <td align="center">' + comment  + '</td></td> </tr>'
print  '<tr><td><input type="radio" checked name=loader id="grub4dos_mbr" value="/boot/Install_MagOS.bat"></td>'
print '<td>grub4dos</td><td>.../boot/Install_MagOS.bat</td><td>Yes</td><td>standard bootloader</td></tr>'
print  '<tr><td><input type="radio" name=loader id="grub4dos" value="/boot/grub4dos/install.lin/bootinst.sh"></td>'
print '<td>grub4dos</td><td>.../boot/grub4dos/install.lin/bootinst.sh</td><td></td><td></td></tr>'
print  '<tr><td><input type="radio" name=loader id="syslinux" value="/boot/syslinux/install.lin/bootinst.sh"></td>'
print '<td>syslinux</td><td>.../boot/syslinux/install.lin/bootinst.sh</td><td></td><td></td></tr>'
print '</table>'

if len(boot_devs[0][0]) != 0:
	print '<h3>' + disk_header + '</h3>'
	print '<table  id="disk-table" border="1" width="100%">' 
	print '<tr><td align="center">' + flag + '</td><td align="center">' +  device +  '</td><td align="center">' + mount_point + '</td></tr>'
	for dev in boot_devs:
		print  '<tr><td><input type="radio" name="boot-mount" id="' + dev[0].replace('/', '-') + '" value="' + dev[1] + '"></td>'
		print '<td>' + dev[0] + '</td><td>' + dev[1] + '</td></tr>' 
	print '</table>'
