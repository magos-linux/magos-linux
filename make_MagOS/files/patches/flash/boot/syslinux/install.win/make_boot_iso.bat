...
boot\tools\WIN\mkisofs.exe @boot\syslinux\iso_conf -o "%ISOPATH%\%CDLABEL%.iso" -A "%CDLABEL%" -V "%CDLABEL%" -m '*.xzm' -m '*.lzm' -m '*_save*' -m '*.sgn' MagOS=MagOS boot=boot isolinux.cfg=boot/syslinux/syslinux.cfg
echo.
