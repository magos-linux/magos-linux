...
--- a/nv.c	2013-09-18 20:34:04.000000000 +0400
+++ b/nv.c	2013-09-18 21:27:56.000000000 +0400
@@ -148,7 +148,6 @@ static int      nvos_post_vbios(void *,
...
@@ -190,12 +189,34 @@ void          nv_kern_rc_timer(unsigned
...
@@ -490,6 +511,12 @@ static struct pci_dev* nv_get_pci_device
     return NULL;
 }
...
@@ -509,19 +536,19 @@ static void nvos_proc_create(void)
     nv_state_t *nv;
...
@@ -546,24 +573,19 @@ static void nvos_proc_create(void)
             break;
...
+            entry = proc_mkdir_mode("agp", d_flags, proc_nvidia);
             if (!entry) {
                 NV_PCI_DEV_PUT(dev);
                 goto failed;
@@ -574,72 +596,78 @@ static void nvos_proc_create(void)
 #endif
...
@@ -648,41 +676,18 @@ nvos_proc_add_text_file(
 )
 {
 #ifdef CONFIG_PROC_FS
...
@@ -2852,17 +2857,18 @@ void   NV_API_CALL  nv_set_dma_address_s
 }
...
+    nv = (nv_state_t *) m->private;
     ...
     if (!dev)
@@ -2873,61 +2879,94 @@ nv_kern_read_cardinfo(char *page, char *
...
+    nv = (nv_state_t *) m->private;
     ...
     if (nv) {
         dev = nv_get_pci_device(nv);
@@ -2938,12 +2977,12 @@ nv_kern_read_agpinfo(char *page, char **
         if (!dev)
             return 0;
...
@@ -2957,39 +2996,73 @@ nv_kern_read_agpinfo(char *page, char **
...
+    nv = (nv_state_t *) m->private;
     ...
     if (!dev)
@@ -3011,10 +3084,10 @@ nv_kern_read_status(char *page, char **s
     command &= scratch;
...
@@ -3022,17 +3095,17 @@ nv_kern_read_status(char *page, char **s
         if (status & 0x8) // agp 3.0
             agp_rate <<= 2;
...
@@ -3046,7 +3119,7 @@ nv_kern_read_status(char *page, char **s
...
@@ -3054,37 +3127,63 @@ nv_kern_read_status(char *page, char **s
     }
...
+
  ...
  ***/
diff -auprN a/nv-i2c.c b/nv-i2c.c
--- a/nv-i2c.c	2013-06-24 18:43:29.000000000 +0400
+++ b/nv-i2c.c	2013-09-18 21:28:12.000000000 +0400
...
@@ -298,15 +296,10 @@ BOOL NV_API_CALL nv_i2c_del_adapter(nv_s
...
--- a/nv-linux.h	2013-06-24 18:43:29.000000000 +0400
+++ b/nv-linux.h	2013-09-18 21:28:21.000000000 +0400
@@ -128,6 +128,7 @@
...
+
 #endif  /* _NV_LINUX_H_ */
diff -auprN a/os-interface.c b/os-interface.c
--- a/os-interface.c	2013-06-24 18:43:29.000000000 +0400
+++ b/os-interface.c	2013-09-18 21:27:01.000000000 +0400
@@ -261,7 +261,7 @@ NvU64 NV_API_CALL os_get_page_mask(void)
